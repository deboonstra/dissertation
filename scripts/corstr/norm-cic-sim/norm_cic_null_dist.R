...
# CIC(R) statistic. The null distribution will be characterized by the CIC(R)
...
res_list <- paste0(
  ...
  c(
    "norm_cic_sim.rds",
    "norm-cic-beta5-sim/norm_cic_beta5_sim.rds",
    "norm-cic-rho05-sim/norm_cic_rho05_sim.rds",
    "norm-cic-rho05beta5-sim/norm_cic_rho05beta5_sim.rds",
    "norm-cic-rho70-sim/norm_cic_rho70_sim.rds",
    "norm-cic-rho70beta5-sim/norm_cic_rho70beta5_sim.rds"
  )
)
...
names(res) <- substr(
  x = res_list,
  start = regexpr(pattern = "cic_", text = res_list, perl = TRUE),
...
for (k in seq_along(res)) {
  ...
  colnames(res_cs) <- c("simulation", "N", "n", "corstr", "cic")
  colnames(res_ar1) <- c("simulation", "N", "n", "corstr", "cic")
  ...
  res[[k]] <- subset(
    x = res[[k]],
    select = c(simulation, dim_beta, alpha, N, n, corstr, cic)
  )
}
