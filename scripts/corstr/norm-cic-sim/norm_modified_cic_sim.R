...
# properties of a modified version of CIC. For information about this
# simulation, see the meeting notes for 2023-09-07 and the document explaining
# this simulation: `outputs/norm_modified_cic_sim/norm_modified_cic_sim.Rmd`.
...
for (j in seq_len(nsims)) {
  ...
  for (k in seq_along(work_corstr)) {
    ...
    ### Getting CIC values ####
    cic_cs[k] <- cic(fit_cs, modified = TRUE)
    cic_ar1[k] <- cic(fit_ar1, modified = TRUE)

    #### Getting true CIC values ####

    ##### Exchangeable
    omega_cs <- solve(fit_cs$geese$vbeta.naiv)
    res_cic0_cs[j, k] <- sum(diag(omega_cs %*% sigma0_cs))

    ##### AR(1)
    omega_ar1 <- solve(fit_ar1$geese$vbeta.naiv)
...
if (!dir.exists("./outputs/corstr/norm-cic-sim/norm-modified-cic-sim")) {
  dir.create("./outputs/corstr/norm-cic-sim/norm-modified-cic-sim")
}
...
save(
  ...
  file = paste0(
    ...
    "norm-modified-cic-sim/norm_modified_cic_sim.RData"
  )
)