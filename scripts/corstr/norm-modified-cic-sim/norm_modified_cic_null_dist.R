...
# mCIC(R) statistic. The null distribution will be characterized by the mCIC(R)
...
res_list <- list.files(
  path = "./outputs/corstr/norm-modified-cic-sim",
  pattern = ".rds",
  recursive = TRUE,
  full.names = TRUE
)
...
names(res) <- substr(
  x = res_list,
  start = regexpr(pattern = "modified_cic_", text = res_list, perl = TRUE),
...
for (k in seq_along(res)) {
  ...
  colnames(res_cs) <- c("simulation", "N", "n", "corstr", "mcic")
  colnames(res_ar1) <- c("simulation", "N", "n", "corstr", "mcic")
  ...
  res[[k]] <- subset(
    x = res[[k]],
    select = c(simulation, dim_beta, alpha, N, n, corstr, mcic)
  )
}
...
for (j in seq_len(nrow(null_dist))) {
  ...
  dist_info <- MASS::fitdistr(x = x, densfun = "normal")

  # Filling the basis data.frame ####
  null_dist$mean[j] <- dist_info$estimate[1]
  null_dist$var[j] <- dist_info$estimate[2]^2
}
