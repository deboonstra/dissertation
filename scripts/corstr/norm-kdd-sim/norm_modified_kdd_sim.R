...
# properties of modified KDD. For information about this simulation, see the
# meeting notes for 2023-09-21 and the document explaining this simulation:
# `outputs/norm-modified-kdd-sim/norm_modified_kdd_sim.Rmd`.
...
for (j in seq_len(nsims)) {
  ...
  for (k in seq_along(work_corstr)) {
    ...
    ### Getting kl values
    kl_cs[k] <- kl(fit_cs, modified = TRUE)
    kl_ar1[k] <- kl(fit_ar1, modified = TRUE)
  }
...
if (!dir.exists("./outputs/corstr/norm-kdd-sim/norm-modified-kdd-sim")) {
  dir.create("./outputs/corstr/norm-kdd-sim/norm-modified-kdd-sim")
}
...
save(
  ...
  file = paste0(
    ...
    "norm-modified-kdd-sim/norm_modified_kdd_sim.RData"
  )
)