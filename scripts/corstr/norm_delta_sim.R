...
for (j in seq_len(nsims)) {
  ...
  for (k in seq_along(work_corstr)) {
    ...
    delta_cs[k] <- delta(a = fit_cs$geese$vbeta.naiv, b = fit_cs$geese$vbeta)
    delta_ar1[k] <- delta(a = fit_ar1$geese$vbeta.naiv, b = fit_ar1$geese$vbeta)

    #### Getting true delta values ####

    ##### Exchangeable
    res_delta0_cs[j, k] <- delta(a = fit_cs$geese$vbeta.naiv, b = sigma0_cs)

    ##### AR(1)
    res_delta0_ar1[j, k] <- delta(a = fit_ar1$geese$vbeta.naiv, b = sigma0_ar1)
  }
