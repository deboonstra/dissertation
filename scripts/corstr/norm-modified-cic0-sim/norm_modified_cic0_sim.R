...
subdir <- "./outputs/corstr/norm-modified-cic0-sim/"
  ...
  res[[ell]] <- parallel::parLapply(
    ...
    fun = function(j) {
      ...
      for (k in seq_along(work_corstr)) {
        ...
        omega <- MASS::ginv(fit$naive.variance, tol = .Machine$double.eps)
        cc[k] <- sum(diag(x = crossprod(x = omega, y = sigma0[[ell]])))
...
saveRDS(
  object = res,
  file = paste0(subdir, "norm_modified_cic0_sim.rds")
)