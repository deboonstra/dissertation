...
# the selection properties and distribution of Kullback-Leibler symmetric
# divergence. This simulation is an extension of the simulations ran in
# norm_modified_ksd_sim, as we are using the model-based covariance matrix
...
sub_dir <- "./outputs/corstr/norm-ksd-sim/norm-kd-symmetric-sim/"
  ...
  res[[ell]] <- parallel::parLapply(
    ...
    fun = function(j) {
      ...
      for (k in seq_along(work_corstr)) {
        ...
        kb[k] <- kd(object = fit, symmetric = TRUE)
      ...
      if (all(!is.na(kb))) {
        ...
        tr_flip <- sum(diag(crossprod(x = invert(sigma0[[ell]], tol = tol), y = mb))) #nolint

        # Calculating KL divergence
        kb0 <- 0.5 * (tr + tr_flip) - k
      } else {
        kb0 <- NA
      }
...
saveRDS(
  object = res,
  file = paste0(sub_dir, "norm_kd_symmetric_sim.rds")
)