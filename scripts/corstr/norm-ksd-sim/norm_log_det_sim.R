...
# properties of the logarithm of the determinants. For information about this
# simulation, see the meeting notes for 2023-09-28 and the document explaining
# this simulation: `outputs/norm-log-det-sim/norm_log_det_sim.Rmd`.
...
for (j in seq_len(nsims)) {
  ...
  for (k in seq_along(work_corstr)) {
    ...
    ### Getting ld values
    ld_cs[k] <- log_det(fit_cs)
    ld_ar1[k] <- log_det(fit_ar1)
  }
...
if (!dir.exists("./outputs/corstr/norm-ksd-sim/norm-log-det-sim")) {
  dir.create("./outputs/corstr/norm-ksd-sim/norm-log-det-sim")
}
...
save(
  ...
  file = "./outputs/corstr/norm-ksd-sim/norm-log-det-sim/norm_log_det_sim.RData"
)
