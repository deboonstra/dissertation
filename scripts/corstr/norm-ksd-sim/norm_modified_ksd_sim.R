...
# properties of modified KSD. For information about this simulation, see the
# meeting notes for 2023-09-21 and the document explaining this simulation:
# `outputs/norm-modified-ksd-sim/norm_modified_ksd_sim.Rmd`.
...
for (j in seq_len(nsims)) {
  ...
  for (k in seq_along(work_corstr)) {
    ...
    ### Getting kl values
    kl_cs[k] <- kl(fit_cs, type = "ksd", modified = TRUE)
    kl_ar1[k] <- kl(fit_ar1, type = "ksd", modified = TRUE)
  }
...
if (!dir.exists("./outputs/corstr/norm-ksd-sim/norm-modified-ksd-sim")) {
  dir.create("./outputs/corstr/norm-ksd-sim/norm-modified-ksd-sim")
}
...
save(
  ...
  file = paste0(
    ...
    "norm-modified-ksd-sim/norm_modified_ksd_sim.RData"
  )
)