...
# properties of the inverted logarithm of the determinants. For information
# about this simulation, see the meeting notes for 2023-10-05 and the document
# explaining this simulation:
# `outputs/norm-inv-log-det-sim/norm_inv_log_det_sim.Rmd`.
...
for (j in seq_len(nsims)) {
  ...
  for (k in seq_along(work_corstr)) {
    ...
    ### Getting ld values
    ld_cs[k] <- -1 * log_det(fit_cs)
    ld_ar1[k] <- -1 * log_det(fit_ar1)
  }
...
if (!dir.exists("./outputs/corstr/norm-ksd-sim/norm-inv-log-det-sim")) {
  dir.create("./outputs/corstr/norm-ksd-sim/norm-inv-log-det-sim")
}
...
save(
  ...
  file = pasteo(
    ...
    "norm-inv-log-det-sim/norm_inv_log_det_sim.RData"
  )
)