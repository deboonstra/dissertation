...
# properties of KSD. For information about this simulation, see the meeting
# notes for 2023-09-21 and the document explaining this simulation:
# `outputs/norm_ksd_sim/norm_ksd_sim.Rmd`.
...
for (j in seq_len(nsims)) {
  ...
  for (k in seq_along(work_corstr)) {
    ...
    ### Getting kl values
    kl_cs[k] <- kl(fit_cs, type = "ksd")
    kl_ar1[k] <- kl(fit_ar1, type = "ksd")
  }
...
if (!dir.exists("./outputs/corstr/norm-ksd-sim")) {
  dir.create("./outputs/corstr/norm-ksd-sim")
}
...
save(
  ...
  file = "./outputs/corstr/norm-ksd-sim/norm_ksd_sim.RData"
)
