...
for (j in seq_len(nsims)) {
  ## Simulating data ####
  dat <- sim_data2(N = N, n = n, beta = beta, rho = rho)
  y <- dat$y
  X <- dat$X
  id <- dat$id
  ## Transform data ####
  ...
  wc <- rep(
    c("independence", "exchangeable", "ar1", "unstructured"),
    each = length(bs_temp)
  )
  ## Fitting models ####
...
save(
  ...
  file = paste0(
    "./simulations/explore/",
    "norm_glsx_mix_qic_corstr_sim_data2_sim/",
    "norm_glsx_mix_qic_corstr_sim_data2_sim.RData"
  )
)