...
- For the covariates,
    ...
    - each $x_{ij,k}$ for $k = 2 \ldots 6$ are generated from $Uniform(j, j + 1)$ and $\log$-tranformed.
...
```
and should be compared to `norm_bqicu_sim` to see the impact the transformation process has on the selection of the mean and correlation structures. Additionally, these results are by-product of an updated version of the `sim_data` function. The development version of this function is stored in the `simulations/explore` directory. 
``` {r}
sim_data2
```
