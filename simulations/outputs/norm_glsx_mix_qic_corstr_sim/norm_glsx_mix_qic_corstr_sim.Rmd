...
## Best subsets
All of the $(2^6 - 1) \times 3 = 189$ subsets of the fully saturated model with the three correlation structures being investigated were fit using either generalized linear models (GLMs) or generalized estimating questions (GEEs). Then, the mean and correlation structure pairing that resulted in the minimum information criterion (IC) value was selected as the best fitting models. These results are different from `norm_glsx_bqicu_sim` because with those simulations the correlation structure was **not** selected. Additionally, the goodness-of-fit (GOF) term of IC is obtained using the transformed response and covariate matrix while the penalty term is calculated using the original data.
...
Based on these results, it appears that the correlation structure was selected appropriately every single time regardless of likelihood framework used to define the goodness-of-fit term.

### Joint
Thus, on average, the models, with the minimum information criteria, have mean structures that includes all the non-zero coefficients at minimum and the proper correlation structure.
```{r}
joint <- function(x) {
    p <- function(y) {
        if (length(y) == 0) {
    ...
    return(dplyr::bind_rows(ip, ep, ap))
}

data.frame(
    lik = c("Quasi-likelihood", "", "", "Likelihood", "", ""),
    corstr = rep(c("Independence", "Exchangeable", "AR(1)"), 2),
