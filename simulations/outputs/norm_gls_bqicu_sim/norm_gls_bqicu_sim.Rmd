...
# Simulations for `norm_gls_bqicu_sim.R`
...
### Best subsets
Using the transformed response, all $2^6 - 1 = 63$ the subsets of the fully saturated model were fit using either an independence, exchangeable, or AR(1) working correlation structure. Then, the model resulting in the minimum QICu and BQICu value were selected as the best fitting models.
...
```{r loading sim res}
load("./simulations/outputs/norm_gls_bqicu_sim/norm_gls_bqicu_sim.RData")
```

### Feature selection based on QICu and BQICu
...
Using the `look` function, we have the following summary measures from the simulation set, where the models resulting in the minimum QICu and BQICu values were selected as the best fitting model for each working correlation structure.
...
### QICu and BQICu across the simulations
Now for each of the models with the minimum QICu and BQICu, the information criteria values will be broken into their parts to see what is impacting the selection process. Additionally, this provides a look at the stability of the information criterions across the simulations. QICu and BQICu have the same goodness-of-fit term (GOF) as QIC (2001a)
$$
GOF = -2 * \text{quasi-likelihood},
$$
which assumes independence among the response to allow the use of the quasi-likelihood framework. However, QICu and BQICu replace the penalty term of QIC with $2p$ and $\log(N)p,$ respectively, where $p$ is the number of parameters in the model.
