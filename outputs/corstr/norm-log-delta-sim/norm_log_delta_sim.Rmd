...
subtitle: "given the maximum variance ratio is log-transformed"
...
where $\hat{\boldsymbol{\Sigma}}_{min}$ is the model-based covariance matrix associated with the working correlation matrix that produced the minimum $\delta$ statistic and $\boldsymbol{\Sigma}_{0}$ is the estimated true covariance matrix. *These $\delta$ values are based on the maximum variance ratio being log-transformed. Thus, the minimum value is 0.*

$$
\delta\left(\hat{\boldsymbol{\Sigma}}, \hat{\mathbf{V}}_{r}\right) = \sum_{i = 1}^{d} log\left\{max\left(\frac{\hat{\mathbf{V}}_{r, ii}}{\hat{\boldsymbol{\Sigma}}_{ii}}, \frac{\hat{\boldsymbol{\Sigma}}_{ii}}{\hat{\mathbf{V}}_{r, ii}}\right)\right\}
$$
...
```{r, echo = FALSE, fig.align = "center", out.width = "100%"}
# plotting delta values
plot_all(data = res_delta, index = fixed_n, min_val = 0)
...
```{r, echo = FALSE, fig.align = "center", out.width = "100%"}
# plotting delta values
plot_all(data = res_delta, index = fixed_obs, min_val = 0)
