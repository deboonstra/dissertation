...
We are making these comparisons because previous research usually only focuses on exchangeable and AR(1). As these simulations are meant to investigate the selection properties of $\delta$, for each comparison, the correlation structure that resulted in the minimum number of correlation parameters estimated that has a $\delta$ value within Occam's window was selected as the best performing correlation structure. Occam's window was defined by calculating the 90th percentile of the sampling distribution of $\delta$. Additionally, Occam's window was implemented for each comparison outline above. Considering the exchangeable and AR(1) correlation structures have identical number of correlation parameters estimated, the correlation structure that has the minimum $\hat{\delta}$ value will be selected.

- Additionally, if there are no criterion values that fall within Occam's window, then the correlation structure associated with the minimum $\hat{\delta}$ is selected.

### Sampling distribution of $\delta$
In defining Occam's window for each of the model selection comparisons, the sampling distribution of $\delta$ was used. In these simulations, prior to peforming any model selection procedure the sampling distribution of $\delta$ was approximated by randomly generating 1,000 data sets assuming within-subject independence and obtaining $\hat{\delta}$ for the largest correlation structure under consideration.
...
- For the second, third, and fourth model selection comparisons, the sampling distributions are based on the unstructured correlation structure, as that represents thel largest and most general correlation structure.
- Currently, the randomly generated data is based on solely on $N, n, \text{ and } p$, the length of the mean structure plus the intercept term. Additionally, it is assumed there is **NO** within-subject correlation.
  - $\mathbf{X}[, 1] = 1$ for the intercept
  - $\mathbf{X}[, 2:p] \sim Unif(-1, 1)$ for the other parameters
  - $\boldsymbol{\beta} = 1 \text{  } \forall \mathbf{X}[, j]$ for $j = 1, \ldots , p$.
  - $\mathbf{y} = \mathbf{X}\boldsymbol{\beta} + \boldsymbol{\varepsilon},$ where $\varepsilon_{i} \sim N(0, \mathbf{I}_{n})$ for $i = 1, \ldots , N$.
...
res <- readRDS(file = paste0(sub_dir, "norm_delta_occam_gen_samp_sim.rds"))
```
...
The selection properties of the $\delta$ presented below are based on the data generation process that used an *AR(1)* correlation structure.
