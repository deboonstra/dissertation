...
- Additionally, if there are no criterion values that fall within Occam's window, then the **NO** correlation structure is selected.

### Sampling distribution of $\delta$
In defining Occam's window for each of the model selection comparisons, the sampling distribution of $\delta$ was used. In these simulations, the exact simulation distribution of $\delta$ was not used. Instead, an approximation of the sampling distribution was used based on an observation when examining the sampling distribution.

- The sampling distribution is approximately gamma with a rate parameter approximately equal to the sample size ($N$) and has a mean value approximately equal to the minimum observed $\hat{\delta}$. Thus, the shape parameter of the gamma distribution is approximately equal to $N \times \hat{\delta}$.
