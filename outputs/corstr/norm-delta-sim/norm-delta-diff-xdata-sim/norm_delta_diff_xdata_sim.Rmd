...
subtitle: "using a different data generation mechanism"
...
The results of these simulations are to verify the correlation structure selection properties of a new measure called $\delta$ initially but is now called Variance Information Criteria (VIC).
...
```
For one-thousand replications, the correlated normal responses are generated from the model $$y_{ij} = \mathbf{X}_{ij}^{\top}\boldsymbol{\beta} + \varepsilon_{ij},$$ where $i = 1, \ldots, N,$ $j = 1, \ldots, n,$ $\mathbf{X}_{ij} = (x_{ij, 1}, \ldots, x_{ij, 7})^{\top},$ and $\boldsymbol{\beta} = (\beta_{0}, \beta_{1}, \beta_{2}, \beta_{3}, \beta_{4}, \beta_{5}, \beta_{6})^{\top} = (0.0, 2.0, 3.0, 0.5, 0.0, 0.0, 0.0)^{\top}$, where the combinations of $N$ and $n$ are listed below.
...
- For the covariates,
  - $x_{ij, 1} = 1 \forall i, j$ to represent the intercept term,
  - $x_{ij, 2}$ was generated from the Bernoulli(0.5) distribution, and
  - $x_{ij, k}$ for $k = 3, \ldots, 7$ was generated from the uniform distribution, where:
    - if $k$ is an odd number then  $x_{ij, k} = -1 \times Unif(j, j + 1)$,
    - else if $k$ is an even number then $x_{ij, k} = Unif(j, j + 1)$.
...
- This data generation mechanism was implemented using the `sim_norm()` *R* function.
...
res <- readRDS(file = paste0(sub_dir, "norm_delta_diff_xdata_sim.rds"))
```
...
```{r, echo = FALSE, fig.align = "center", out.width = "100%"}
# plotting delta values
plot_all(data = res_delta, index = fixed_n, min_val = l)
...
```{r, echo = FALSE, fig.align = "center", out.width = "100%"}
# plotting delta values
plot_all(data = res_delta, index = fixed_obs, min_val = l)
