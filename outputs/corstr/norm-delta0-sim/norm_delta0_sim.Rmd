...
3. exchangeable versus AR(1) versus independence versus unstructured.
...
```{r}
res <- readRDS(file = "./outputs/corstr/norm-delta0-sim/norm_delta0_sim.rds")
```
...
res_plot$group <- factor(res_plot$n)
...
ggplot2::geom_vline(
  xintercept = l + 1, linetype = "dotted", color = "firebrick", linewidth = 1
) +
ggplot2::labs(
...
ggplot2::guides(fill = ggplot2::guide_legend(title = "N = 200, n =")) +
ggplot2::theme_bw() +
...
ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5, size = 10))
ggpubr::ggarrange(
  independence_plot, exchangeable_plot, ar1_plot, unstructured_plot,
...
mean_tab <- knitr::kable(
  ...
  caption = "Mean values for Fixed N"
)
...
var_tab <- knitr::kable(
  ...
  col.names = c("N = 200, n =", legend_work_corstr),
  caption = "Variance values for Fixed N"
)
...
res_plot <- subset(
  x = res_plot,
  subset = group %in% group_id
)
res_plot$group <- factor(x = res_plot$group, levels = unique(res_plot$group))
...
mean_tab <- knitr::kable(
  ...
  caption = "Mean values for Fixed total number of observations"
)
kableExtra::kable_styling(kable_input = mean_tab, full_width = FALSE)
...
var_tab <- knitr::kable(
  x = var_tab, format = "html", digits = 3,
  col.names = c("N = , n =", legend_work_corstr),
  caption = "Variance values for Fixed total number of observations"
)
kableExtra::kable_styling(kable_input = var_tab, full_width = FALSE)
```
...
##### Structure selection comparison 1
...
res_sel$group <- factor(res_sel$n)
...
res_sel$group <- factor(x = res_sel$group, levels = unique(res_sel$group))
...
##### Structure selection comparison 2
...
##### Structure selection comparison 3

For structure selection comparison 1, we are considering all the working correlation structures of *independence*, *exchangeable*, *AR(1)*, and *unstructured*.
...
For structure selection comparison 1, we are only considering the working correlation structures of *independence*, *exchangeable*, and *AR(1)*.
