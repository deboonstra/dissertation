...
\end{aligned}
$$
where $KDD(Z, Y)$ is related to `kdd(object1 = mod_z, object2 = mod_y)` and $KDD(Y, Z)$ is related to `kdd(object1 = mod_y, object2 = mod_z)`. **While the KDD-eo simulation investigated expected optimism being estimated with the original fitting model ($Y = X_{1} + X_{2} + X_{3}$), in this simulation expected optimism was estimated with an interecept only model.** This change was made because the replicated data was generated by an intercept only model and using the original fitting model to estimate expected optimism is misspecifying the mean structure.
```{r, echo = FALSE}
